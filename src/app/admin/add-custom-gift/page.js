...
export default function AddCustomGift() {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingGiftId
      ? `/api/custom-gifts/${editingGiftId}`
      : "/api/custom-gifts";
    ...
    if (res.ok) {
      showToast(`Custom Gift ${editingGiftId ? "updated" : "added"} successfully!`, "success");
      setForm({
        name: "",
        price: "",
        description: "",
      ...
      showToast("Failed to save custom gift", "error");
    }
  };
  ...
  const handleEdit = (gift) => {
    ...
    setForm({
      ...
      customDetails: gift.customDetails || "",
      category: gift.category || "custom-gift",
      images: gift.images || [],
    });
  ...
  const handleDelete = async (id) => {
    ...
    const giftName = gift.name || "this custom gift";

    // Show immediate feedback
    showToast(`Deleting "${giftName}"...`, "deleting");

    try {
      const res = await fetch(`/api/custom-gifts/${id}`, { method: "DELETE" });
      ...
      <div className="flex-1">
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4 md:p-6">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="mb-12">
              <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-cyan-500/20">
                <h2 className="text-3xl font-bold mb-6 text-white">
                  {editingGiftId ? "Edit Custom Gift" : "Add New Custom Gift"}
                </h2>

                <form onSubmit={handleSubmit} className="space-y-6">
                  {/* Gift Name */}
                  <div>
                    <label className="block text-slate-300 mb-2 font-medium">Custom Gift Name</label>
                    <input
                      name="name"
                      placeholder="Enter custom gift name"
                      value={form.name}
                  ...
                  {/* Custom Details */}
                  <div>
                    <label className="block text-slate-300 mb-2 font-medium">Custom Details</label>
                    <textarea
                      name="customDetails"
                      placeholder="Enter customization details, special requirements, personalization options, etc."
                  ...
                  >
                    {editingGiftId ? "Update Custom Gift" : "Add Custom Gift"}
                  </button>
                </form>
              </div>
            </div>
            ...
            {/* Custom Gifts List */}
            <div className="bg-slate-800/30 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-cyan-500/20">
              <h2 className="text-3xl font-bold mb-6 text-white">All Custom Gifts</h2>

              {loading ? (
                <div className="text-center py-8">
                  <div className="w-8 h-8 border-2 border-cyan-300 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-slate-400">Loading custom gifts...</p>
                </div>
              ) : gifts.length === 0 ? (
                <div className="text-center py-8">
                  <div className="text-6xl mb-4 text-slate-500">🎨</div>
                  <p className="text-slate-400 text-lg">No custom gifts found</p>
                  <p className="text-slate-500">Add your first custom gift above</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {gifts.map((gift) => (
                    ...
                    >
                      ...
                      {gift.customDetails && (
                        <div className="mb-3">
                          <span className="inline-block bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-300 text-xs font-medium px-3 py-1 rounded-full">
